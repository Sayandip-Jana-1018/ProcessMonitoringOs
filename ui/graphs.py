...
def update_performance_graphs(axes, timestamps, cpu_history, mem_history, disk_history, theme):
    ...
    try:
        ...
        # CPU Graph (Red theme)
        axes[0].plot(x, cpu_history, color='#FF4757', linewidth=2, label='CPU')
        axes[0].fill_between(x, 0, cpu_history, color='#FF4757', alpha=0.2)
        
        # Memory Graph (Blue theme)
        axes[1].plot(x, mem_history, color='#2E86DE', linewidth=2, label='Memory')
        axes[1].fill_between(x, 0, mem_history, color='#2E86DE', alpha=0.2)
        
        # Disk Graph (Green theme)
        axes[2].plot(x, disk_history, color='#26C281', linewidth=2, label='Disk')
        axes[2].fill_between(x, 0, disk_history, color='#26C281', alpha=0.2)
        ...
        for ax in axes:
            ...
            # Add legend
            ax.legend(loc='upper right', facecolor=theme["chart_bg"], 
